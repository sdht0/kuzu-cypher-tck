...
Feature: Match9 - Match deprecated scenarios
  ...
  @modified @outputModified @kuzuFuncMissing
  # Kuzu missing last()
  Scenario: [1] Variable length relationship variables are lists of relationships
    ...
    When executing query:
      """
      MATCH ()-[r*0..1]-()
      RETURN list_transform(rels(r), i->concat('[:', LABEL(i), ']')) as l
      """
  ...
  @modified @outputModified @keywordClash
  ...
  @modified @outputModified @keywordClash @testbug @fails @extraOutputUndirected
  ...
  @modified @outputModified @testbug @fails @extraOutputUndirected
  ...
  Scenario: [9] Optionally matching named paths with variable length patterns
    ...
    When executing query:
      ...
      RETURN list_transform(rels(r), i->concat('[:', LABEL(i), ']')) as r,
             concat('({name: \'', x.name, '\'})') as x,
